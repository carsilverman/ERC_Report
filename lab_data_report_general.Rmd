...
#specify datasets to analyze
datasets <- c('walkin_data/F16data.csv', 'walkin_data/S17data.csv')

#specify semester names
semesters <- c('Fall 2016', 'Spring 2017')

#specify cleaning functions; for future semesters, replicate clean_s17.R and make any necessary changes
clean <- c('clean_scripts/clean_f16.R','clean_scripts/clean_s17.R')

#specify graduate and undergraduate fellow names (as they appear in the qualtrics survey)
...
#read in the files
files <- lapply(datasets, read.csv)

#prep cleaning functions
lapply(clean, source)
clean_fun <- gsub('.R', "", clean)
clean_fun <- gsub('clean_scripts/', "", clean_fun)
...
The code currently runs on 1 or more semesters of data.
...
Rather than obtaining counts for specific majors, it is more useful to group our visitors into three categories based on major: Social Sciences, STEM, and Humanities. Visitors who are double majors or who specify a minor in the survey are counted twice in the analysis.
...
We display tables and plots for both individual fellows and fellow "type", that is undergraduate or graduate student.
...
analyze_fellow <- function(lab_data){
  ...
  #grad assistants and undergrad fellows are specified at the top of the script
...
The Day of Week plot displays the number of visitors to the ERC on each day for the semesters provided.
...
#Plot
for(i in 1:2) {major_s[[i]][[1]]$school <- names(major_s)[i]}
...
#Plot
for(i in 1:2) {program_s[[i]][[1]]$school <- names(program_s)[i]}
...
t <- xtable(x, caption = 'Frequency of Number of Visits per Student by Semester', align=paste(c('cc', rep('c', n)), collapse = ''), digits = c(rep(0,n+2)))
print(t, include.rownames=FALSE, sanitize.colnames.function = large, booktabs = T)
```
